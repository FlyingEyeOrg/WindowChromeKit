...
#### `TitleBarPalette` 的三套 Element Plus 配色

与上面的预置配色**并列**的另一轴（`TitleBarStyle` 之外的第二个属性）。
**每套的颜色固定、不随样式变化** —— 这是回归时要重点确认的：
...
- [ ] 同一套配色配到三套骨架上，颜色**完全相同**（实测 9 种组合，每列三行一致）
- [ ] 三套配色互不相同（主色 / 深色 / 中性的底色两两不同）
