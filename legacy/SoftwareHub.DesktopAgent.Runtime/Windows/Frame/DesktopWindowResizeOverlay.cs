...
internal sealed class DesktopWindowResizeOverlay : IDisposable
{
    internal const string WindowClassName = "SoftwareHub.DesktopAgent.ResizeOverlay";
    ...
    private static readonly Dictionary<IntPtr, DesktopWindowResizeOverlay> Instances = [];
    ...
    internal DesktopWindowResizeOverlay(IntPtr owner, Func<NativePoint, bool> isCaptionButton)
    {
        ...
        if (_handle == IntPtr.Zero)
            throw new Win32Exception(Marshal.GetLastWin32Error(), "无法创建 DesktopAgent resize overlay HWND。");
    ...
    internal void Synchronize()
    {
        ...
        if (!NativeWindowMethods.GetWindowRect(_owner, out var bounds))
            throw new Win32Exception(Marshal.GetLastWin32Error(), "无法读取 DesktopAgent 主窗口矩形。");
        ...
        {
            throw new Win32Exception(Marshal.GetLastWin32Error(), "无法同步 DesktopAgent resize overlay。");
        }
    ...
    private int HitTest(IntPtr longParameter)
    {
        ...
        if (!NativeWindowMethods.GetWindowRect(_handle, out var bounds)) return DesktopWindowFrameHitTest.Client;
        var (borderX, borderY) = GetResizeBorder();
        ...
        int borderY)
    {
        ...
        if (left && top) return DesktopWindowFrameHitTest.TopLeft;
        if (right && top) return DesktopWindowFrameHitTest.TopRight;
        if (left && bottom) return DesktopWindowFrameHitTest.BottomLeft;
        if (right && bottom) return DesktopWindowFrameHitTest.BottomRight;
        if (left) return DesktopWindowFrameHitTest.Left;
        if (right) return DesktopWindowFrameHitTest.Right;
        if (top) return DesktopWindowFrameHitTest.Top;
        if (bottom) return DesktopWindowFrameHitTest.Bottom;
        return DesktopWindowFrameHitTest.Client;
    }
    ...
    private static bool IsResizeHit(int hit) =>
        hit is >= DesktopWindowFrameHitTest.Left and <= DesktopWindowFrameHitTest.BottomRight;
    ...
    private static void RegisterWindowClass()
    {
        lock (ClassGate)
        {
            ...
            {
                throw new Win32Exception(Marshal.GetLastWin32Error(), "无法注册 DesktopAgent resize overlay 窗口类。");
            }
