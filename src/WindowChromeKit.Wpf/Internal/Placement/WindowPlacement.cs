...
internal static class WindowPlacement
{
        ...
        NativeRectangle? ownerRectangle = null)
    {
        ...
        left = Clamp(left, target.WorkArea.Left, Math.Max(target.WorkArea.Left, target.WorkArea.Right - width));
        top = Clamp(top, target.WorkArea.Top, Math.Max(target.WorkArea.Top, target.WorkArea.Bottom - height));
