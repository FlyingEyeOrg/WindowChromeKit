...
internal static class ElementPlusPalettes
{
    ...
    private static readonly Color Danger = Color.FromRgb(0xF5, 0x6C, 0x6C);    // color-danger
    ...
    /// <c>IsExternalInit</c>（WinForms 样例是 net8.0，可以用 record）。
    /// </summary>
    ...
    internal static Palette For(ChromeTitleBarStyle style) => style switch
    {
        ...
        ChromeTitleBarStyle.VsCode => new Palette(
            activeCaption: Frozen(DarkBg),
            inactiveCaption: Frozen(DarkOverlay),
            captionText: Frozen(Mix(TextBase, DarkBg, 0.95)),          // text-color primary
            inactiveCaptionText: Frozen(Mix(TextBase, DarkBg, 0.65)),  // text-color secondary
            buttonHover: Frozen(Mix(FillBase, DarkBg, 0.12)),          // fill-color ''
            buttonPressed: Frozen(Mix(FillBase, DarkBg, 0.20)),        // fill-color darker
            closeButtonHover: Frozen(Danger),
            closeButtonPressed: Frozen(Mix(White, Danger, 0.20)),      // 深色主题 dark-2 = 混白
        ...
        ChromeTitleBarStyle.Windows => new Palette(
            activeCaption: Frozen(White),
            inactiveCaption: Frozen(Color.FromRgb(0xF2, 0xF6, 0xFC)),  // fill-color extra-light
            captionText: Frozen(Color.FromRgb(0x30, 0x31, 0x33)),      // text-color primary
        ...
        _ => new Palette(
            activeCaption: Frozen(Primary),
            inactiveCaption: Frozen(Mix(White, Primary, 0.30)),        // primary light-3
            captionText: Frozen(White),
    ...
    /// <summary>
    /// 把配色套到窗口上。**必须在 <see cref="ChromeWindow.TitleBarStyle"/> 之后调用** ——
    /// 赋值样式时会一次性套用整张样式表（几何 + 配色），先改颜色会被它覆盖回去。
    /// </summary>
    internal static Palette Apply(ChromeWindow window, ChromeTitleBarStyle style)
    {
        ...
        window.ActiveTitleBarBackground = palette.ActiveCaption;
        window.InactiveTitleBarBackground = palette.InactiveCaption;
        window.ActiveTitleBarForeground = palette.CaptionText;
        window.InactiveTitleBarForeground = palette.InactiveCaptionText;
        window.CaptionButtonHoverBackground = palette.ButtonHover;
        window.CaptionButtonPressedBackground = palette.ButtonPressed;
        window.CloseButtonHoverBackground = palette.CloseButtonHover;
        window.CloseButtonPressedBackground = palette.CloseButtonPressed;
